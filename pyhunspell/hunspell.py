...
class Hunspell():
    ...
    def _load_attr(self):
        try:
            get_dic_cmd = self.cmd + ['-D']
            p = subprocess.Popen(get_dic_cmd, shell=False, 
                                 stdin=open('/dev/null'),
                                 stderr=subprocess.STDOUT,
                                 stdout=subprocess.PIPE)
        except OSError as e:
            raise OSError("%s failed: errno=%d %s" % (get_dic_cmd, e.errno, e.strerror))
        self.attr = {}
        header = ''
        while True:
            ...
            if not len(line):
                break
            # Match a section start
            # AVAILABLE DICTIONARIES (path is not mandatory for -d option):
            m = re.match('([A-Z]+\s[A-Z]+).*:$', line)
            if m:
                # Create section using header name
                header = m.group(1)
                self.attr[header] = []
            elif len(header):
                # Fill real data of the section
